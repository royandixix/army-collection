<nav class="navbar navbar-expand-lg shadow-sm fixed-top"
    style="background: linear-gradient(90deg, #1e3c72, #2a5298); z-index:1040;">
            ...
            @auth
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end shadow mt-2" style="width: 300px;">
                        <li class="px-3 pt-2 text-muted small">Notifikasi</li>
                        @forelse($notifications as $notif)
                            <li>
                                <a href="{{ route('user.riwayat.index') }}" class="dropdown-item d-flex align-items-center gap-2">
                                    @if (isset($notif->data['bukti_pengiriman']))
                                        <img src="{{ asset('storage/' . $notif->data['bukti_pengiriman']) }}" width="40"
                                            height="40" class="rounded border" style="object-fit: cover;">
                                    @endif
                        ...
                        <li>
                            <form action="{{ route('user.notifikasi.bacaSemua') }}" method="POST" class="d-inline w-100">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="dropdown-item text-center text-primary">
                                    Tandai semua dibaca
                                </button>
                            </form>
                        </li>
                ...
                <div class="dropdown ms-2">
                    <a href="#" class="d-flex align-items-center text-white text-decoration-none dropdown-toggle"
                        ...
                        <img src="{{ Auth::user()->img ? asset('storage/' . Auth::user()->img) : asset('img/default-user.png') }}"
                            alt="Foto Profil" width="40" height="40" class="elegant-avatar">
                    </a>
    ...
    <div class="container d-flex justify-content-end">
        <form action="{{ route('user.produk.index') }}" method="GET"
            ...
            <input type="text" name="search"
                class="form-control form-control-sm border-0 shadow-none px-2 search-input" placeholder="Cari produk..."
...
@push('scripts')
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const phrases = [
                "Temukan gaya terbaikmu.", "Tampil keren tanpa usaha lebih.", "Belanja mudah, tampil maksimal.",
